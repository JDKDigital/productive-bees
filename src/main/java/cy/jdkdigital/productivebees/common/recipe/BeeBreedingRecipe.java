...
{
    public static final MapCodec<BeeBreedingRecipe> MAP_CODEC = RecordCodecBuilder.mapCodec(
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, BeeBreedingRecipe> STREAM_CODEC = StreamCodec.of(
            BeeBreedingRecipe::toNetwork, BeeBreedingRecipe::fromNetwork
    );

    public static final RecipeSerializer<BeeBreedingRecipe> SERIALIZER = new RecipeSerializer<>(MAP_CODEC, STREAM_CODEC);
    ...
    public boolean matches(RecipeInput inv, Level worldIn) {
        if (inv instanceof BeeHelper.IdentifierInventory) {
            Identifier beeId1 = BeeRegistries.resolveId(Identifier.parse(((BeeHelper.IdentifierInventory) inv).getIdentifier(0)));
            Identifier beeId2 = BeeRegistries.resolveId(Identifier.parse(((BeeHelper.IdentifierInventory) inv).getIdentifier(1)));
            for (Supplier<BeeIngredient> parent : List.of(parent1, parent2)) {
                if (parent.get() != null) {
                    Identifier parentId = BeeRegistries.resolveId(parent.get().getBeeType());
                    if (!parentId.equals(beeId1) && !parentId.equals(beeId2)) {
                        return false;
                    }
                } else {
    ...
    @Override
    public RecipeSerializer<BeeBreedingRecipe> getSerializer() {
        return SERIALIZER;
    }

    @Override
    public RecipeType<BeeBreedingRecipe> getType() {
