...
public class HatModel<T extends ProductiveBeeEntity> extends EntityModel<T>
{
    ...
    public void setRotationAngles(T entity, float v1, float v2, float v3, float v4, float v5) {
        this.santaHat.rotateAngleX = 0.0F;
        this.santaHat.rotationPointY = 19.0F;
        boolean flag = !entity.isAirBorne && entity.getMotion().lengthSquared() < 1.0E-7D;
        if (!flag) {
            this.santaHat.rotateAngleX = 0.0F;
        ...
        float lvt_8_2_;
        if (!((BeeEntity)entity).func_233678_J__()) {
            this.santaHat.rotateAngleX = 0.0F;
            this.santaHat.rotateAngleY = 0.0F;
            this.santaHat.rotateAngleZ = 0.0F;
            if (!flag) {
