...
public class BlockConversionRecipeCategory implements IRecipeCategory<BlockConversionRecipe>
{
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, BlockConversionRecipe recipe, IFocusGroup focuses) {
        ...
        } else if (recipe.stateFrom.getFluidState().getType().equals(Fluids.EMPTY) && recipe.fromDisplay.isPresent()) {
        ...
        if (recipe.stateTo.getFluidState().getType().equals(Fluids.EMPTY) && recipe.toDisplay.isPresent()) {
    ...
    public void draw(BlockConversionRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        ...
        guiGraphics.drawString(minecraft.font, Language.getInstance().getVisualOrder(Component.translatable("jei.productivebees.block_conversion.chance", recipe.chance)), 0, 45, 0xFF000000, false);
    }
}
