...
public class CentrifugeRecipeCategory implements IRecipeCategory<CentrifugeRecipe> {
    ...
    public CentrifugeRecipeCategory(IGuiHelper guiHelper) {
        ResourceLocation location = new ResourceLocation(ProductiveBees.MODID, "textures/gui/container/centrifuge.png");
    ...
    public void setRecipe(@Nonnull IRecipeLayout recipeLayout, @Nonnull CentrifugeRecipe recipe, @Nonnull IIngredients ingredients) {
        ...
        itemStacks.init(0, true, 38, 35);
        itemStacks.init(1, true, 76, 17);

        int startX = 68;
        int startY = 8;
        IntStream.range(2, outputs.size()+1).forEach((i) -> {
            if (i > 11) return;
