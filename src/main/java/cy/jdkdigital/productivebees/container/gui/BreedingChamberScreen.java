...
//            tooltipList.add(new TranslatableComponent("productivebees.breeding_chamber.tooltip.progress", (this.menu.tileEntity.getProcessingTime() - this.menu.tileEntity.getRecipeProgress()) / 20).getVisualOrderText());
//        }

        // Up arrow
        if (isHovering(159 - 13, 14, 10, 10, mouseX, mouseY)) {
            tooltipList.add(new TranslatableComponent("productivebees.breeding_chamber.tooltip.next_bee").getVisualOrderText());
        }
        // Down arrow
        if (isHovering(159 - 13, 27, 10, 10, mouseX, mouseY)) {
            tooltipList.add(new TranslatableComponent("productivebees.breeding_chamber.tooltip.prev_bee").getVisualOrderText());
        }
        ...
        if (isHovering(85 - 13, 14, 18, 18, mouseX, mouseY)) {
            this.menu.tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
                if (handler.getStackInSlot(BreedingChamberContainer.SLOT_CAGE).isEmpty()) {
                    tooltipList.add(new TranslatableComponent("productivebees.breeding_chamber.tooltip.cage").getVisualOrderText());
                }
            });
        }
